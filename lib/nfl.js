...
export function selectWeeklyGames(events) {
  ...
  // Select 1 MNF game if scheduled
  const chosenMNF = mnfGames.length > 0 
    ? mnfGames[Math.floor(Math.random() * mnfGames.length)] 
    : null;

  // Shuffle all non-Monday games (Wednesday, Thursday, Friday, Saturday, Sunday)
  const shuffledOthers = [...otherGames].sort(() => 0.5 - Math.random());

  // Fill the first 4 slots (or 5 if an off-week has no Monday game)
  const targetOtherCount = chosenMNF ? 4 : 5;
  const chosenOthers = shuffledOthers.slice(0, targetOtherCount);

  // Sort games 1 through 4 chronologically by kickoff date/time
  chosenOthers.sort((a, b) => new Date(a.date) - new Date(b.date));

  // Anchor Monday Night Football cleanly at slot 5
